...
V2 (distributed):
┌──────────┐  ┌───────────┐  ┌─────────┐
│Orchestr. │  │Model Runn.│  │Evaluator│
└──────────┘  └───────────┘  └─────────┘
      └────── Redis Streams ─────┘

V3 (horizontal):
┌──────────┐  ┌───────────┐  ┌───────────┐
│Orchestr. │  │Model Run#1│  │Model Run#2│
└──────────┘  └───────────┘  └───────────┘
      └────────── Redis ──────────┘
```

The architecture stays the same — only the deployment changes.

---

## MVP Scope

**Components:** All 5 (API Gateway, Orchestrator, Model Runner, Evaluator, Aggregator)

**Voting Modes:** Hierarchical only

**Evaluation Strategies:** Chairman Review only (simplest)

**Aggregation:** Winner Takes All only

**Provider:** OpenRouter only

**Event Bus:** In-memory queue (for simplicity)

**Adding new modes:** Just create a new class and register it
